...
RSpec.describe "Device Readings", type: :request do
  ...
  describe "POST /", parse_json: true do
    ...
    it "creates readings" do
      ...
      expect(store).to have_cache_key("#{params[:id]}_latest_timestamp").with_value("#{params[:id]}_#{readings.second[:timestamp]}")
    ...
    it "assigns latest timestamp" do
      ...
      expect(store).to have_cache_key("#{params[:id]}_latest_timestamp").with_value("#{params[:id]}_#{soonest}")
